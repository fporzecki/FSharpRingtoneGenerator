...
namespace Assembly
    module Assembler =
        ...
        let assembleToPackedStream (score:string) (bpm:float) =
            match (parse score bpm) with
        ...
        let produce (score:string) (filename:string) (bpm:float) =
            match (assembleToPackedStream score bpm) with
                | Choice2Of2 ms -> 
